    ...
    >
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4">
          ...
          {/* Meta - حذف دوبار MapPin */}
          <div className="mt-2 flex flex-wrap gap-3 text-[12px] text-foreground-muted">
            ...
            {product.location && (
              <span className="flex items-center gap-1 text-foreground-muted/90">
                {product.location}
              </span>
            )}
            <span className="flex items-center gap-1">
          ...
          <div className="mt-4 rounded-lg bg-background-subtle p-4">
            <p className="text-md leading-7 text-foreground-muted whitespace-pre-wrap break-words text-center">
          ...
          <div className="mt-4 flex items-center gap-3 rounded-lg border border-border p-4">
            ...
            <div className="flex-1">
              <p className="text-[10px] font-bold text-foreground-muted">فروشنده</p>
              <p className="text-sm font-bold text-foreground">{product.seller_name}</p>
            </div>
          </div>

          {/* Security */}
          <div className="mt-4 rounded-xl bg-primary-soft p-3 ring-1 ring-primary/10">
            <div className="flex items-center gap-2 text-primary-text">
              <ShieldCheck className="h-4 w-4" />
              <p className="text-xs font-medium">اطلاعات تماس محافظت شده است</p>
            </div>
          </div>
        </div>

        {/* ===== Bottom Actions ===== */}
        <div className="border-t border-border p-4 space-y-2 bg-card/50 backdrop-blur-sm">
          <div className="grid grid-cols-2 gap-3">
            {product.status === 'approved' && !product.isSold && (product.quantity || 0) > 0 ? (
              <>
                <button className="btn btn-primary w-full text-sm h-12 font-bold shadow-lg shadow-primary/20">
                  خرید امن
                </button>
                {onAddToCart && (
                  <button
                    onClick={handleAddToCart}
                    className="btn btn-secondary w-full gap-2 text-sm h-12 font-bold"
                  >
              ...
              <button className="col-span-2 btn btn-secondary w-full text-sm h-12 font-bold cursor-not-allowed opacity-60">
                {product.isSold
                  ? 'فروخته شده'
                  : product.quantity === 0
                    ? 'ناموجود'
